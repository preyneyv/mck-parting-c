...
static inline void ti_start(TimingInstrumenter *ti) {
  ti->start_time = platform_time_now();
}

// Stop and accumulate
static inline void ti_stop(TimingInstrumenter *ti) {
  ti->end_time = platform_time_now();
  ti->aggregate_time += platform_time_diff_us(ti->start_time, ti->end_time);
  ti->count++;
}

// Last measured interval (µs)
static inline uint64_t ti_get_elapsed_us(const TimingInstrumenter *ti) {
  return platform_time_diff_us(ti->start_time, ti->end_time);
}
