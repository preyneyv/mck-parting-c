...
// void *audio_playback_main() {
//   while (1) {
//     uint32_t *buf = audio_buffer_pool_acquire_write(&pool, true);
//     // printf("[writer] got buffer!\n");
//     sleep_ms((rand() % 30)); // Simulate audio processing
//     audio_buffer_pool_commit_write(&pool);
//     // printf("[writer] gave buffer!\n");
//   }
// }
...
void *audio_thread_main() {
  ...
  // pthread_create(&audio_playback, NULL, audio_playback_main, NULL);

  // audio_buffer_pool_init(&pool, 5, 256);

  // printf("Audio thread running...\n");
  // while (1) {
  //   uint32_t *buf = audio_buffer_pool_acquire_read(&pool, true);
  //   audio_buffer_pool_commit_read(&pool);
  //   sleep_ms(100);
  //   // sleep_ms(1000); // Simulate audio processing
  // }
