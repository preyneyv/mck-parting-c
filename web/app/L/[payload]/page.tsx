...
export default async function ScoreIngestPage({ params }: { params: Promise<{ payload: string }> }) {
  ...
  return <div className="mx-auto max-w-lg"><Card><CardHeader><CardTitle>Submit {entry.gameName} score</CardTitle><CardDescription>{entry.summary}</CardDescription></CardHeader><CardContent><ScoreSubmitForm payload={payload} deviceSerial={entry.deviceSerial} initialName={initialName} /></CardContent></Card></div>;
}
