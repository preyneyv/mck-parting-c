...
uint8_t u8x8_bitmap_SetSize(u8x8_bitmap_t *b, uint16_t pixel_width,
  ...
  b->u8g2_buf =
      b->u8x8_buf + (size_t)b->tile_width * (size_t)b->tile_height * (size_t)8;
...
static void tga_write_byte(FILE *fp, uint8_t byte) { fputc(byte, fp); }
...
static uint8_t u8x8_SetBitmapDeviceSize(U8X8_UNUSED u8x8_t *u8x8,
                                        uint16_t pixel_width,
                                        uint16_t pixel_height) {
...
static void u8x8_DrawBitmapTiles(U8X8_UNUSED u8x8_t *u8x8, uint16_t tx,
                                 uint16_t ty, uint8_t tile_cnt,
                                 uint8_t *tile_ptr) {
...
static uint8_t u8x8_d_bitmap(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int,
  ...
  case U8X8_MSG_DISPLAY_INIT:
    u8x8_d_helper_display_init(
        u8x8); /* update low level interfaces (not required here) */
    break;
